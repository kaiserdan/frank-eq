...
## 2026-08-12 — terminate v3-2 with a negative decision and fail-closed verifier refusal

- **Machine decision:** `decision.json` records status `fail` and diagnosis `ONE_SHOT_PUBLIC_BASIS_NOT_QUALIFIED`. Semantic-basis and unseen-renderer checks fail; behavioral basis, public alignment, held-sender retention, quantization, oracle execution, and integrity pass. Composition and activation-specificity also fail. All eleven named integrity checks are true, and receiver-protocol drafting, receiver execution, new receiver-world access, scientific claims, and paper claims remain false.
- **Terminal state:** all ordered workflow stages complete and the decision, metrics, rate/compute, prediction manifest, artifact manifest, and independent audit are written. The independent audit verifies 118 artifact files, all registered test hashes, the exact decision, rate/compute, integrity, and closed authorizations, but records `metrics_recomputed_exactly=false`; the workflow therefore fails closed with `RuntimeError: independent Stage-A v3 audit failed`, and Olivia job `1899057` terminates `FAILED`/exit `1:0` after 12:31:24.
- **Exact-runtime localization:** non-outcome diagnostic job `1953471` runs the frozen reducer under the same ARM64 image and NumPy 2.2.6 on `gpu-1-109`, completing 0:0 in 13 seconds. Loading bundles in workflow/config order reproduces stored metric SHA-256 `10dd92543939fabefef26e19a09263d1bef157fc526f6867f306bce286f45e41` exactly with zero differences. Loading the same hash-verified bundles in the verifier's lexicographic manifest-key order produces 46 numeric-only differences, maximum absolute delta `5.551115123125783e-17`, while reproducing `decision.json` exactly.
- **Classification:** the Slurm failure is a deliberate fail-closed verifier-order-sensitivity refusal, not congestion, OOM, CUDA failure, capture failure, access violation, gate ambiguity, or scientific disagreement. The v3-2 outcome remains the frozen negative machine decision; no same-registration rerun, adaptive repair, threshold change, receiver work, or claim is authorized. Full fetch and independent package verification remain required before compact evidence adoption.

## 2026-08-12 — adopt the compact Stage-A v3-2 negative

- **Fetch:** repository fetch preserves `runs/` and `logs/` under ignored `.agents/state`. A checksum-only remote/local `rsync` comparison over the complete 26.45-GB tree and logs emits no difference and exits zero.
- **Verification:** runner verification finds all 28 required top-level artifacts and stops on terminal workflow state `failed`. The specialized verifier checks 118 manifest artifacts and all 21 registered test artifacts, reproduces six prediction groups, every integrity check, the exact machine decision, and exact rate/compute. It correctly remains failed on terminal workflow state, the original failed audit, metric byte inequality, and post-failure artifact-manifest lifecycle.
- **Manifest lifecycle:** all 118 original manifest entries exist and 117 hashes match. Only `workflow_status.json` changes after manifest creation, from completed pre-audit hash `f664a9a0...2c113` to fail-closed terminal hash `dd67c7c3...cf5ba`; `independent_audit.json` is written after the pre-audit manifest and cannot be added because the audit refuses promotion. No scientific artifact differs.
- **Strata:** all six semantic groups beat the train prior and clear balanced accuracy, but Brier remains `0.1547`--`0.1719` against the frozen `0.10` ceiling. Every unseen-renderer gain interval is negative. Behavioral basis passes all groups. Aggregate composition is positive over prior and direct, but held/model and `mutual` strata fail the all-strata contract. Token-ID activation specificity fails; public alignment, held retention, four-bit retention `0.9961`, and zero oracle mismatches pass.
- **Evidence:** adopt the compact hash-manifested package under `evidence/real_stagea_v3_olivia/`, including the original machine artifacts, verification summary, exact-runtime order diagnostic, and independent audit narrative. Exclude raw captures, checkpoints, prediction arrays, logs, caches, and operational state from Git.
- **Authority:** `ONE_SHOT_PUBLIC_BASIS_NOT_QUALIFIED` is terminal for v3-2. No current experiment, receiver protocol, receiver execution, receiver-world access, scientific claim, or paper claim is authorized. Any future question requires separate scientific justification and a fresh registration; exposed v3 worlds, models, renderers, metrics, and thresholds cannot be used for confirmation selection.

## 2026-08-12 — freeze and merge the Stage M0 operation-closure audit

- **Scientific distinction:** the v3 public packet contains first-order edge marginals, while nonlinear operation probabilities generally require joint events because `E[f(E)] != f(E[E])`. Register Stage M0 as a new development-only diagnostic of public-state completeness, not a v3 retry, compiler rescue, receiver experiment, or claim-bearing confirmation.
- **Frozen contract:** `docs/23_STAGE_M_OPERATION_CLOSED_BASIS.md` and `configs/moment_compute/real_olivia_m0.yaml` bind one Olivia `full` / `audit` run over 64 four-entity development worlds, 32 operations, two renderers, disjoint calibration/selection/validation roles, and pinned Qwen3-4B/Qwen3-8B founders. There is no held sender or test role.
- **Operation closure:** generate the typed event registry from the complete operation grammar, preserve the historical marginal/independence executor as a control, and compare the exact affine moment executor with both that control and a cross-fitted direct protocol. Calibrate by model, event kind, and event order, never event ID.
- **Pre-execution repair:** before any model run, correct the historical counterfactual grammar so the added edge is genuinely load-bearing: add `u->v` and query two-hop reachability from `u` to a distinct `w`. Rebuild the intervention event registry and tests rather than preserving the degenerate four-distinct-argument form.
- **Static evidence:** validation reports 318 typed coordinates, event-registry SHA-256 `70ce5d31d22e814b91ca0f1f6ac29567e2ef4447b472c426a979166471ca6d55`, contract SHA-256 `769fbf6519fe991112f3cfb9de7b3871e264e76b0fdd9c2cee5cb394889c8326`, zero exact-executor mismatches, and all protected authorizations closed. This is implementation evidence only; no Stage M model outcome exists.
- **Merge and authority:** merge commit `659c120` installs the frozen protocol, implementation, verifier, CI/static validation, runner skill, and Olivia routing on `main`. The immediate next executable is the content-addressed dry run for job `frank-eq-moment-compute-m0`; bind `FRANK_EQ_IMAGE_SHA256` to inspected digest `a3ca46f0...aa3b1` and reject a null plan value. Only `OPERATION_CLOSED_MOMENT_BASIS_SUPPORTED` may authorize drafting a separately frozen successor compiler protocol; it never authorizes launching that protocol, a held sender, receiver work, or a claim.

## 2026-08-12 — launch the frozen Stage M0 Olivia audit

- **Validation and source:** launch from clean commit `d4e64bb78b6cce8740a0856bb971a9492fa662e1` after compile, Ruff, 114 tests, shell syntax, repository validation, rate--compute validation, and moment--compute validation pass. The independently audited 252-file source archive has SHA-256 `352215a620e2b9147140a719c8c4ad8666a3cacf51d5d73d1454b5c2ccc27668`.
- **Frozen bindings:** config SHA-256 `f181fece5f47078c9aae3a04195fd44156efa917648f6f0591192e948d80d51d`, image SHA-256 `a3ca46f0db9971b4108e5c8694e72f3039166383efc06b01f4031183208aa3b1`, event-registry SHA-256 `70ce5d31d22e814b91ca0f1f6ac29567e2ef4447b472c426a979166471ca6d55`, and contract SHA-256 `769fbf6519fe991112f3cfb9de7b3871e264e76b0fdd9c2cee5cb394889c8326` are exact.
- **Submission:** run only profile `full` and stage `audit` as Olivia Slurm job `1970800`, job name `frank-eq-moment-compute-m0`, on one H200 with 32 CPUs, 128 GiB memory, and a 12-hour limit. The scheduler starts it immediately on `gpu-1-58` at `2026-08-12T21:26:56+02:00`.
- **Startup audit:** the in-job static validator passes with 64 four-entity worlds, 32 operations, 318 event coordinates, two pinned founder models, zero exact-executor mismatches, and protected authorizations closed. The deployed archive and config independently hash to the submitted values; no model outcome or machine decision is read at this checkpoint.
- **Authority:** this is the sole frozen Stage M0 execution. It remains development-only and has no held sender, test role, receiver access, or claim authority. Monitor to termination, fetch, verify independently, and audit all registered strata before interpretation; even a pass permits only drafting a separately frozen successor protocol.

## 2026-08-13 — complete Stage M0 with an event-readout negative

- **Terminal execution:** Olivia job `1970800` completes `0:0` in `01:37:52` after all 128 prefixes per model, 52,992 records per model, and 1,824 compiled validation predictions. Exact Qwen revisions, exclusive cloned-KV batching, zero replay, disjoint 32/13/19 development roles, 318 event coordinates, and zero formal-executor mismatches are preserved.
- **Machine decision:** `decision.json` records `fail` and `OPERATION_CLOSED_EVENTS_NOT_READABLE`. Joint out-degree and two-path-intersection groups fail the frozen balanced-accuracy lower bound for both Qwen3-4B and Qwen3-8B; every other event group passes.
- **Composition:** moment Brier is `0.05310`, versus marginal/independence `0.02744` and cross-fitted direct `0.19445`. The lower-95 gain is `-0.02999` over marginals and `0.11980` over direct. Both model strata fail against marginals; exact atomic retention passes.
- **Authority:** all protected fields remain false, including `successor_protocol_draft_authorized`. Follow the registered event-readout stop rule: stop the current graph/source line and do not train a one-shot compiler, rerun M0, enlarge the basis, open a held/receiver role, or make a claim.

## 2026-08-13 — verify and adopt the Stage M0 negative

- **Fetch and primary verification:** fetch `runs/` and `logs/` through the repository launcher. Checksum-only remote/local comparisons emit no differences. The repository verifier passes, and the original in-job independent verifier reproduces artifact hashes, roles, registry identity, 1,824 predictions, metrics, decision, workflow state, and closed authorizations exactly.
- **Cross-runtime refusal:** the workstation Python 3.14 / NumPy 2.5.2 verifier changes 96 prediction rows only in repeated projection-adjustment diagnostics: 48 mean fields by at most `6.94e-18` and 48 maximum fields by at most `4.44e-16`. Every scientific prediction field, all metrics, and the machine decision remain exact.
- **Exact-runtime confirmation:** a fresh temporary Python 3.10 / NumPy 2.2.6 environment matching the run reproduces all predictions, metrics, and the decision byte-for-byte and passes the specialized verifier. Classify the initial refusal as cross-version diagnostic reduction sensitivity, not a scientific disagreement.
- **Evidence:** adopt the compact SHA-256-manifested package under `evidence/real_stage_m_olivia_m0/`. Exclude raw 126-MiB response trees, predictions, panels, W&B state, logs, caches, credentials, and operational state from Git. Repository validation pins the source/image lineage, gate vector, failed event groups, causal branch metadata, verification, and authorization boundary.
- **Next state:** no experiment or cluster executable is authorized. Any continuation begins with a new task-level scientific question and separately frozen fresh roles; drafting that question is not execution authority.

## 2026-08-13 — freeze the fresh SPQ0 predictive-state census

- **New question:** leave the consumed graph line and test a shared predictive quotient on fresh full-support controlled stochastic systems. The historical unexecuted PSR0 branch is code provenance only; reject its stochastic true/false scoring.
- **Protocol:** use exact Qwen3-4B and Mistral-7B-v0.3 founders, reserve exact OLMo2 and Granite revisions unopened, forecast ten categorical probability bins, select an exact typed rank-four future-test core from fit systems only, and keep calibration/selection/validation roles with no test role.
...
## 2026-08-13 — execute and terminate SPQ0 with a source-protocol negative

- **Completed outcome:** after separate operator launch authority, the sole complete census runs as `frank-eq-spq0-olivia-20260813c`, Olivia Slurm `2006680`, from clean commit `2a0b2044f1d3e3926fc7ca2b5158e8ad5abab156`. It completes `0:0` in `00:51:22` on one GH200. Earlier job `2006621` is a pre-inference Python-3.10 startup failure; repaired job `2006673` is operator-cancelled before a complete outcome. Neither contributes a scientific result.
- **Integrity:** each active founder produces 2,880 capture rows, 92,160 categorical future-test branches, 92,160 exact prefix checks, and 46,080 exact event-boundary checks with literal cloned KV, exclusive batching, and zero replay. Calibration/selection/validation stay disjoint; test histories, pair-specific mappers, held-sender rows, and receiver rows remain zero. OLMo2/Granite snapshot resolution, file open, adapter, load, tokenizer, and inference counters are zero.
- **Machine decision:** `decision.json` records `fail` and `SOURCE_PROBABILITY_PROTOCOL_NOT_QUALIFIED`. Both founders lose badly to the target prior under every source-protocol stratum. Semantic core decoding passes on seen data but fails the OOD/specificity conjunction. Mistral-to-Qwen transfer passes, while Qwen-to-Mistral fails oracle-reader retention and rank localization; sender identity remains above ceiling and the residual census selects rank zero.
- **Non-promotional positives:** preserve exact public-basis construction, the Mistral-to-Qwen directional positive, and four-bit retention in both directions. They cannot override the frozen conjunctive gate. Positive amortized utility is relative to the failed direct categorical protocol and is not a communication claim.
- **Authority:** every protected field is false, including `spq1_protocol_draft_authorized`. Do not retune or rerun SPQ0, access reserved checkpoints, draft or execute SPQ1, open held/test/receiver roles, or make a claim.

## 2026-08-13 — verify and adopt the SPQ0 negative

- **Exact-runtime verification:** the in-job Olivia verifier live-rehashes both active snapshots, validates 32 artifact entries, regenerates systems and panels, refits every probe/reader, recomputes predictions, metrics, rate/compute, and the exact machine decision, and passes with maximum numeric delta `0.0` at `1e-12` tolerance.
- **Cross-runtime refusal:** the workstation Python 3.14.6 / NumPy 2.5.2 verifier records maximum delta `2.0413661039331288e-09`, localized to Qwen fitted weights, compiled targets, tied selection-method labels, and derived diagnostic fields. Panel actions/observations match; posterior drift is at most `4.44e-16`. Artifact hashes, basis, access contract, gate vector, decision, and authorizations remain exact. Preserve this as a portability refusal rather than altering the consumed artifacts.
- **Evidence:** adopt the compact SHA-256-manifested package under `evidence/real_spq0_olivia/`, including causal/access metadata, fitted summaries, grouped metrics, decision, exact-runtime verification, workstation diagnostic, and independent audit. Exclude generated captures, checkpoint/prediction arrays, panels, logs, W&B state, credentials, and `.agents/state/`.
- **Next state:** SPQ0 is consumed and terminal. There is no current experiment or cluster executable. Any continuation needs a new scientific question, fresh roles, and a separately frozen registration.
